...
class RedemptionClient {
  constructor (partnerId, customerSecret, opts) {
  ...
  recordRedemption (redemption) {
    ...
    const serverEndpointPath = this.options.testMode ? '/perks/redemption/v1.1-test' : '/perks/redemption/v1.1';
    ...
    return this.options.fetch(url, {
      method: 'POST',
      headers: { 'User-Agent': 'UNiDAYS node.js tracking SDK v1.1' }
    });
  }

  toQueryString (redemption) {
    let qs = '?CustomerId=' + encodeStringValue(this.partnerId)
      + '&TransactionId=' + encodeStringValue(redemption.transactionId)
      + '&Currency=' + encodeStringValue(redemption.currency)
      ...
      let sig = generateSignature(qs, this.customerSecret);
      qs = qs + '&Signature=' + encodeStringValue(sig);
  ...
  getTrackingPixelUrl (redemption) {
    ...
    const pixelEndpointPath = this.options.testMode ? '/perks/redemption/v1.1-test.gif' : '/perks/redemption/v1.1.gif',
  ...
  getJsTrackingUrl(redemption) {
    if (redemption == null) throw new Error('redemption is null or undefined');

    const jsEndpointPath = this.options.testMode ? '/perks/redemption/v1.1-test.js' : '/perks/redemption/v1.1.js',
          hostname = 'tracking.myunidays.com';
